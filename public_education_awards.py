...
#   if u: return u[0]!="-"
#   else: return False
# with open("C:\\Users\\Akylo\\Desktop\\inokentiy\\AUCA\\users.txt", "r", encoding="utf-8") as f: users = {int(u.split(" /|\\ ")[0]): u.split(" /|\\ ")[1] for u in f.read().split("\n") if check_sub(u)}
# timer *= 60
# with open("C:\\Users\\akylo\\Desktop\\ruslan\\phrases.txt", "r", encoding="utf-8") as f: data = f.read().split("\n")
# credit = [(data[sub], float(data[sub+1])) for sub in range(0, len(data), 2)]
# # credit = [("Всемирная история искусства", 2.0), ("Математика", 5.5), ("Русская литература", 1.0), ("Химия", 1.5), ("Иностранный язык", 4.0), ("Граждановедение", 1.0), ("География", 1.0), ("Всеобщая история", 1.5), ("Основы креативного и критического мышления", 2.5), ("Физика", 2.5), ("Русский язык", 2.5), ("Информатика", 3.0), ("История Кыргызстана", 1.5), ("Биология", 1.0), ("Кыргызская литература", 1.0), ("Кыргызский язык", 2.5)]#, ("Спортивное программирование 1 группа", "+0.2")]
...
#     else:
#       t = sub[int(t)-1]
#       with open(f"C:\\Users\\Akylo\\Desktop\\inokentiy\\AUCA\\allsyl\\{t}.txt", "r", encoding="utf-8") as f: show(m, t, f.read().split("\n"))
...
#     return 0
#   run = True
#   try: name, password = t.split("@auca.kg")
#   except:
#     answer(m, "Нормально введи, пж")
#     run = False
#     return 0
...
#   with open(f"C:\\Users\\Akylo\\Desktop\\inokentiy\\AUCA\\{name}.txt", mode="w", encoding="utf-8") as f: f.write("\n".join([f"{g[0].strip().ljust(length)}|{str(g[1]).ljust(6)}|{g[2].ljust(3)}|{g[3]}" for g in gs])+f"\n{'GPA'.ljust(length)}|{round((sum([grade_to_gpa[g[2]]*g[3] for g in gs])+2)/general_cred, 1)}")
#   driver.close()
#   driver = []
#   with open(f"C:\\Users\\Akylo\\Desktop\\inokentiy\\AUCA\\{name}.txt", "rb") as f: bot.send_document(m.chat.id, f.read(), visible_file_name="grades.txt")
#   with open(f"C:\\Users\\Akylo\\Desktop\\inokentiy\\AUCA\\{name}.txt", mode="w", encoding="utf-8") as f:pass
#   run = False
